...
def category(request, category):
    courts = Court.objects.filter(type__code=category)[:20]

    template = loader.get_template('test_app/search/category.html')
    context = {
        'type': category,
        'courts': courts,
    }
...
def search(request):
    ...
    if request.method == 'GET':
        ...
        if rate:
            courts = courts.filter(rate__lte=rate)
    ...
    template = loader.get_template('test_app/search/search.html')
